// Vercel Function: Get qualified teams for knockout round display
  ...
  try {
    ...
    if (!matches || matches.length === 0) {
      return res.status(200).json({ 
        qualified: [],
        message: 'No group stage results yet'
      });
    }
    ...
    // Get qualified teams: top 2 from each group + best 8 third-placed
    const qualified = [];
    const thirdPlace = [];
    ...
    thirdPlace.slice(0, 8).forEach(t => {
      qualified.push({ ...t, position: '3rd' });
    });
