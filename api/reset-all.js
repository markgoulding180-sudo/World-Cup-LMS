...
module.exports = async (req, res) => {
  ...
  if (action === 'simulate_full') {
    ...
    try {
      ...
      // ── Register users in parallel batches of 10 ──
      // If user_count is 0, skip registration (users already registered separately)
      const BATCH_SIZE = 10;
      for (let batch = 0; user_count > 0 && batch < user_count; batch += BATCH_SIZE) {
        const batchPromises = [];
        for (let i = batch + 1; i <= Math.min(batch + BATCH_SIZE, user_count); i++) {
          const email = `simuser${i}@wc2026.test`;
          batchPromises.push(
            supabase.auth.admin.createUser({ email, password: 'Test123456!', email_confirm: true })
              .then(({ data: authData, error: authErr }) => {
                if (authErr || !authData?.user) return null;
                return { user: authData.user, i };
              })
              .catch(() => null)
          );
        }
        ...
        // Batch insert users and entries
        if (validResults.length > 0) {
          const usersToInsert = validResults.map(({ user, i }) => ({
            id: user.id,
            username: `simuser${i}`,
            display_name: `Sim Player ${i}`,
            email: user.email
          }));
          const entriesToInsert = validResults.map(({ user }) => ({
            tournament_id: tournament.id,
            user_id: user.id,
            status: 'active',
            lives_remaining: startLives,
          ...
          await supabase.from('tournament_entries').insert(entriesToInsert);
          summary.usersRegistered += validResults.length;
        }
      }
      ...
      const doGroupResults = async (matchday) => {
        ...
        const lossUserIds = Object.keys(userLosses);
        if (lossUserIds.length > 0) {
          const { data: allEntries } = await supabase
            .from('tournament_entries')
            .select('user_id, lives_remaining, tournament_id')
            .in('user_id', lossUserIds)
            .gt('lives_remaining', 0);

          await Promise.all((allEntries || []).map(async entry => {
            const losses = userLosses[entry.user_id] || 0;
            const newLives = Math.max(0, entry.lives_remaining - losses);
            if (newLives === 0) eliminations++;
            await supabase.from('tournament_entries').update({
              lives_remaining: newLives,
              ...(newLives === 0 ? { status: 'eliminated', eliminated_round: 1 } : {})
            }).eq('user_id', entry.user_id).eq('tournament_id', entry.tournament_id);
          }));
        }
      ...
      const doKORound = async (roundNum) => {
        ...
        if (losingUserIds.length > 0) {
          ...
          await Promise.all((allEntries || []).map(async entry => {
            ...
            await supabase.from('tournament_entries').update({
              lives_remaining: newLives,
              ...(newLives === 0 ? { status: 'eliminated', eliminated_round: roundNum } : {})
            }).eq('user_id', entry.user_id).eq('tournament_id', entry.tournament_id);
          }));
        }
