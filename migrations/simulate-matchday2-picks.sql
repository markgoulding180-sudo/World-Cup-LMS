-- ============================================
-- PART 3: SIMULATE MATCHDAY 2 PICKS
-- Users who haven't been eliminated make 3 more picks
-- ============================================
...
BEGIN
    ...
    LOOP
        -- Get teams already picked by this user in any matchday
        SELECT ARRAY_AGG(team_id) INTO previously_picked
        FROM picks
        WHERE user_id = user_record.user_id;
        
        -- Get available teams for Matchday 2 (not previously picked)
        SELECT ARRAY_AGG(DISTINCT team_id) INTO available_teams
        FROM (
            SELECT home_team_id as team_id FROM matches WHERE matchday = 2
            UNION
            SELECT away_team_id as team_id FROM matches WHERE matchday = 2
        ) teams
    ...
    RAISE NOTICE 'Created Matchday 2 picks';
END $$;

SELECT 
  'Matchday 2 Picks' as check_item, 
